...
const char* error_400_form = "Your request has bad syntax oris inherently impossible to satisfy.\n";
...
// 网站的根目录
const char* doc_root = "/home/ag/NoobWebServer/resources";
...
void addfd(int epollfd, int fd, bool one_shot){
    epoll_event event;
    event.data.fd = fd;
    event.events = EPOLLIN | EPOLLRDHUP;//服务器可以有多种触发模式：边缘、水平等，可以通过配置文件去修改
    if(one_shot) //对端异常断开的话就可以在底层去处理，不需要再提交给上层
    {
        // 防止同一个通信被不同的线程处理
        event.events |= EPOLLONESHOT;
    }
    //向epoll实例（这里是epollfd）中添加或者修改或者删除事件
    epoll_ctl(epollfd, EPOLL_CTL_ADD, fd, &event);
    // 因为这里用的是epoll，要一次性读出所有数据，所以文件描述符应该是非阻塞的
    // 设置文件描述符非阻塞
    setnonblocking(fd);  
}
...
// 解析一行数据,判断依据是\r、\n
http_conn::LINE_STATUS http_conn::parse_line(){//从状态机
    char temp;
    for( ; m_checked_idx < m_read_idx; ++m_checked_idx){
        ...
        if(temp == '\r'){
            if((m_checked_idx + 1) == m_read_idx){
                ...
                m_read_buf[m_checked_idx++] = '\0';
                return LINE_OK;//完整解析到一行
            }
            return LINE_BAD;
        } else if(temp == '\n')  {
            if((m_checked_idx > 1) && (m_read_buf[m_checked_idx - 1] == '\r')){
                //判断m_checked_index > 1是为了后面的下标不出错
...
http_conn::HTTP_CODE http_conn::parse_request_line(char* text){
    //获取请求方法、目标URL、HTTP版本
    //例如，有数据如下：GET /index.html HTTP/1.1， 目标是把各部分解析出来
    m_url = strpbrk(text, " \t"); //检测空格和\t哪个在text中先出现
    if(!m_url){ 
        return BAD_REQUEST;
    }
    ...
    */
    if(strncasecmp(m_url, "http://", 7) == 0){   
        m_url += 7;//192.168.1.1:1/index.html
    ...
    m_check_state = CHECK_STATE_HEADER; //主机状态检查，状态变为检查请求头
    return NO_REQUEST;
}
...
// 主状态机，解析请求
/*使用主从状态机的模式进行解析，从状态机（parse_line）负责读取报文的一行，
主状态机负责对该行数据进行解析，主状态机内部调用从状态机，从状态机驱动主状态机。
每解析一部分都会将整个请求的m_check_state状态改变，
状态机也就是根据这个状态来进行不同部分的解析跳转的*/
...
http_conn::HTTP_CODE http_conn::do_request()
{
    ...
    strcpy(m_real_file, doc_root);//拷贝doc_root至m_real_file
    int len = strlen(doc_root);//计算长度
